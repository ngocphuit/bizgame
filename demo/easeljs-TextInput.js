...
class TextInput extends createjs.Container {
  ...
  _setupListeners() {
    window.addEventListener('click', (e) => {
      ...
      var CWidth = this.stage.canvas.offsetWidth,
            HWidth = parseInt(this.stage.canvas.getAttribute('width').replace(/[^0-9]/, '')),
            CHeight = this.stage.canvas.offsetHeight,
            HHeight = parseInt(this.stage.canvas.getAttribute('height').replace(/[^0-9]/, ''));
      const lX = pX - cX - ((CWidth / (HWidth / 100)) /100) * this.x;
      const lY = pY - cX - ((CHeight / (HHeight / 100)) / 100) * this.y;
  ...
  _selectInput() {
    ...
    this._hiddenInput.style.left = (this.x + (this.stage.canvas.offsetLef) + this._padding) + 'px';
    this._hiddenInput.style.top = (this.y + (this.stage.canvas.offsetTop) + this._padding) - 100 + 'px';
